...
// 페이징 및 제목 검색	
	...
	if (!$_POST[searchText]) {
		$sql = "select * from BOARD WHERE BOARD_TYPE = '자유게시판'";
	}else {
		$sql = "select * from BOARD WHERE BOARD_TYPE='자유게시판' AND BOARD_TITLE LIKE '%".$_POST[searchText]."%'";
	}
	...
	if(isset($_GET['nPage']))
	{
	 ...
	 if (!$_POST[searchText]) {
	 	$query = "SELECT * FROM BOARD WHERE BOARD_TYPE ='자유게시판' ORDER BY BOARD_NUM ASC LIMIT $start_list_query, $list_cnt";
	 }else {
	 	$query = "SELECT * FROM BOARD WHERE BOARD_TYPE='자유게시판' AND BOARD_TITLE LIKE '%".$_POST[searchText]."%' ORDER BY BOARD_NUM ASC LIMIT $start_list_query, $list_cnt";
	 }
...
<body>
	...
	<h2>board</h2>	
		<a href="./libertywrite.php" style="text-align : right";>write</a>
		<br>
		<table cellspacing="0" >
			<tr>
				...
				<td style="width: 10%; text-align: center; border-top: 2px solid #797979;">witer</td>
	...
	?>
			<!-- 글 목록들 -->
				<tr>
					...
					<td style="text-align: left;"><a href="./libertyread.php?num=<?=$num?>"><?=$data[BOARD_TITLE]?></a></td>
	...
	if($i == $nPage){ ?>
     ...
     <? } ?>
		...
		<div id="search">
			<!-- 자기자신을 호출 -->
			<form method="post" action="./libertyboard.php">
