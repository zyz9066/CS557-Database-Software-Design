...
// 페이징 및 제목 검색	
	...
	if (!$_POST[searchText]) {
		$sql = "select * from BOARD WHERE BOARD_TYPE='공지사항'";
	}else {
		$sql = "select * from BOARD WHERE BOARD_TYPE='공지사항' AND BOARD_TITLE LIKE '%".$_POST[searchText]."%'";
	}
	...
	if(isset($_GET['nPage']))
	{
	 ...
	 if (!$_POST[searchText]) {
	 	$query = "SELECT * FROM BOARD WHERE BOARD_TYPE='공지사항' ORDER BY BOARD_NUM ASC LIMIT $start_list_query, $list_cnt";
	 }else {
	 	$query = "SELECT * FROM BOARD WHERE BOARD_TYPE='공지사항' AND BOARD_TITLE LIKE '%".$_POST[searchText]."%' ORDER BY BOARD_NUM ASC LIMIT $start_list_query, $list_cnt";
	 }
	...
	<div>
		...
		<table cellspacing="0" >
			<tr>
				...
				<td style="width: 10%; text-align: center; border-top: 2px solid #797979;">writer</td>
	...
	?>
			<!-- 글 목록들 -->
				<tr>
					...
					<td style="text-align: left;"><a href="./boardread.php?num=<?=$num?>"><?=$data[BOARD_TITLE]?></a></td>
